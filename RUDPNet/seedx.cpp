...
void 
CSeed::Seed(DWORD* pdwRoundKey, BYTE* pbyUserKey)
{
	SeedEncRoundKey(pdwRoundKey, pbyUserKey);
}


//! Seed에서 암호화할때 최소 16 byte가 필요 : pszData는 최소 16byte 이상
void 
CSeed::Encrypt(DWORD* pdwRoundKey , const char* pszData , int nSize)
{
	assert_msg(nSize >= 16 , "Seed에서 암호화할때 최소 16 byte가 필요");
	SeedEncrypt((BYTE*)pszData, pdwRoundKey);
}

void
CSeed::Decrypt(const char* pszData , DWORD* pdwKey)
{
	SeedDecrypt((BYTE*)pszData, pdwKey);
}